...
export const Actualite: React.FC<ActualiteProps> = /* optimize */ block(
  ({addToItems, screenWidth, position}) => {
      ...
      >
        ...
        <div className="flex w-full flex-col p-4 sm:p-6">
          ...
          <p className="font-thunderLC text-sm text-cream-carmen sm:text-lg">
            Chez Carmen, on ne mange pas seulement. On savoure, on partage, on rencontre, on rit, on
            aime, on se sent bien, on refait le monde, bref on vit. S'asseoir à une table de Chez
            Carmen, le "restaurant des abattoirs", c'est plonger dans une ambiance bistrot
            conviviale où les bons produits faits maison
          </p>
        </div>
      </div>
    );
